...
function renderSummary(rankingData, metrics) {
  ...
  if (passRateKpiEl) {
    // Mostrar score promedio en lugar de pass rate
    const avgScore = typeof rankingData.average_score === "number"
      ? rankingData.average_score
      : (metrics.total > 0 ? (metrics.pass / metrics.total) * 100 : 0);
    const scoreVal = avgScore.toFixed(2);
...
function renderBulletChart(rankingData) {
  const avg = typeof rankingData.average_score === "number" ? rankingData.average_score : 0;
  const target = 90;
